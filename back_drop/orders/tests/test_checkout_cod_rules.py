...
class CheckoutCODRules(TestCase):
    def setUp(self):
        ...
        self.p_cod = Product.objects.create(
            name="COD OK", 
            price=100, 
            stock=5, 
            cod_allowed=True,  # Use the correct field name
            supplier=self.s
        )
        self.p_no_cod = Product.objects.create(
            name="No COD", 
            price=50, 
            stock=5, 
            cod_allowed=False,  # Use the correct field name
            supplier=self.s
        )
